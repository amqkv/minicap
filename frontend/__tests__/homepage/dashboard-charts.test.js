...
describe("test the dashboard <Dashboard/>", () => {
    it("does not renders the dashboard", () => {
        const wrapper = shallow(<DashboardCharts role="fake role" />);
        expect(wrapper.find(BarChart)).toHaveLength(0);
    });

    it("renders the Immigration Officer dashboard when it is logged in appropriately, there is a chart and a card", () => {
